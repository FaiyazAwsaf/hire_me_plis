...
**Status legend**
- ✅ **Implemented** — wired up, tested, ready to integrate
- 🚧 **Stub** — route exists but returns `{"status": "not_implemented"}` — do not call from UI yet
- 📋 **Planned** — defined in API contracts, not yet registered as a route
...
| **Base URL (prod)** | `https://api.hiremeplis.com` *(not yet deployed)* |
...
> ⚠️ **IMPORTANT — this endpoint uses `application/x-www-form-urlencoded`, not JSON.**
> The field for email is called `username` (OAuth2 standard). Sending JSON will return a `422`.
...
form.set("username", email);      // note: field is "username", not "email"
form.set("password", password);

const res = await fetch("/auth/login", {
  method: "POST",
  body: form,                     // URLSearchParams, not JSON.stringify
});
```
...
These four endpoints are defined (schemas exist, routes are registered) but return `{"status": "not_implemented"}`. They will be wired up in a later pillar. Do not integrate yet.

When implemented, `/cv/profile` will return the full structured profile (personal info, experience, education, skills, projects, certifications) parsed from the uploaded file. The profile object shape is already finalized — see [CV profile shape](#cv-profile-shape) below.

---

### CV profile shape *(for reference — not yet active)*
...
- **Skill match** (40%) — AI extracts required skills from JD, compares against CV skills section
- **Semantic match** (40%) — embeds JD, finds most similar CV experience chunks via cosine similarity
...
> **Not yet implemented** — wired up in Day 5 (Job Hunter Agent). The route exists and accepts requests but returns `{"status": "not_implemented"}`.

When implemented, this will accept a natural language query and return a ranked list of job cards with fit scores.

**Request** *(final shape — not yet active)*
```json
{ "query": "Find React developer jobs in Dhaka" }
```

**Response `200`** *(final shape — not yet active)*
```json
{
  "results": [
    {
  ...
  "source": "bdjobs | linkedin | jsearch | scraped",
  "total": 10
}
```
...
- General career advice: *"How should I approach my job search?"*
- Readiness check: *"Am I ready for a senior data engineer role?"*
- Gap analysis: *"What skills am I missing for a Google internship?"*
- Roadmap generation: *"Build me a 3-month plan to become job-ready"*
- Cover letter: *"Draft a cover letter for this job posting: [paste JD]"*
...
```json
{
  ...
  "messages": [
    { "role": "user",      "content": "What skills do I have?",   "created_at": "2025-01-15T10:00:00Z" },
    { "role": "assistant", "content": "Based on your CV, you...", "created_at": "2025-01-15T10:00:05Z" }
  ]
}
```
...
> **All endpoints in this section are stubs** — they return `{"status": "not_implemented"}`.
> Routes are registered. Schemas are defined in the API contracts. Implement against the shapes below once the backend team wires them up.

### Applications (Kanban board)

| Method | Endpoint | Description |
|---|---|---|
| GET | `/applications` | List all cards for the current user |
| POST | `/applications` | Create a new card |
| PATCH | `/applications/:id` | Edit card fields (notes, deadline, salary, role, company, url) |
| PATCH | `/applications/:id/status` | Move card between columns — fired on drag-and-drop |
...
| Method | Endpoint | Description |
|---|---|---|
| GET | `/goals` | List all goals, ordered by target date |
...
| Method | Endpoint | Description |
|---|---|---|
| GET | `/calendar/events` | List events in a date range — takes `start` and `end` query params |
| POST | `/calendar/events` | Create an event, optionally linked to a goal |
| PATCH | `/calendar/events/:id` | Reschedule or rename — fired on drag-and-drop |
...
| Method | Endpoint | Description |
|---|---|---|
| GET | `/nudges` | List unread AI-generated nudges |
| PATCH | `/nudges/:id/read` | Mark a nudge as read |
...
`streak_days` — consecutive days on which the user applied to at least one job.
`cv_on_file` — `true` only when a CV with `status: done` exists (i.e., all AI features are ready).
...
| ✅ | POST | `/auth/login` | No | **Form data, not JSON. Field is `username`** |
...
| ✅ | DELETE | `/cv/:id` | Yes | |
| 🚧 | GET | `/cv/profile` | Yes | Not wired yet |
| 🚧 | PUT | `/cv/profile` | Yes | Not wired yet |
| 🚧 | PATCH | `/cv/profile` | Yes | Not wired yet |
| 🚧 | POST | `/cv/export` | Yes | Not wired yet |
| ✅ | POST | `/jobs/score` | Yes | Requires CV `status: done` |
| 🚧 | POST | `/jobs/search` | Yes | Day 5 — not wired yet |
...
| 🚧 | PATCH | `/applications/:id/status` | Yes | Drag-and-drop |
| 🚧 | DELETE | `/applications/:id` | Yes | |
| 🚧 | GET | `/goals` | Yes | |
| 🚧 | POST | `/goals` | Yes | |
| 🚧 | PATCH | `/goals/:id` | Yes | |
| 🚧 | DELETE | `/goals/:id` | Yes | |
| 🚧 | GET | `/calendar/events` | Yes | Needs `start` + `end` query params |
