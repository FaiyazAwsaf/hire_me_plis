...
export function Sidebar() {
  ...
  return (
    <aside 
      className={cn(
        "m-3 mr-0 flex h-[calc(100%-1.5rem)] shrink-0 flex-col rounded-2xl border border-white/12 bg-slate-900/68 text-slate-200 shadow-[0_24px_70px_rgba(15,23,42,0.22)] backdrop-blur-2xl transition-all duration-300 ease-in-out text-left",
        isExpanded ? "w-60" : "w-16"
      )}
    >
      {/* --- LOGO / TOGGLE AREA --- */}
      <div className="flex items-center h-14 px-4 border-b border-white/10 justify-between gap-2 overflow-hidden">
        {isExpanded ? (
          <>
            <span className="font-black text-base tracking-tight truncate text-white animate-in fade-in duration-200">
              Hire Me Plis<span className="text-red-500 font-extrabold"></span>
            </span>
            <Button
              variant="ghost"
              size="icon"
              className="h-8 w-8 rounded-xl border border-white/12 bg-white/10 text-slate-200 shadow-sm backdrop-blur hover:bg-white/18"
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-9 w-9 rounded-xl border border-white/12 bg-white/10 text-slate-200 shadow-sm backdrop-blur hover:bg-white/18 mx-auto shrink-0"
              ...
              {hasChildren ? (
                <button
                  ...
                  className={cn(
                    ...
                    active
                      ? "border border-white/18 bg-white/18 text-white shadow-[0_12px_30px_rgba(15,23,42,0.20)] font-semibold"
                ...
                >
                  ...
                  {!isExpanded && (
                    <div className={cn(
                      "absolute left-0 w-1 h-4 bg-[#818CF8] rounded-r-md transition-opacity opacity-0 scale-0 origin-left",
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          ...
                          childActive
                            ? "border border-white/14 bg-white/14 text-white font-semibold shadow-sm"
